...
// Fetch recipient data with account info
$stmt = $conn->prepare("
    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white border border-green-100 rounded-2xl shadow-xl shadow-green-100/20 overflow-hidden">
            ...
            <div class="bg-green-50/50 border-b border-green-100 px-8 py-6 flex justify-between items-center">
                <div>
                    <h2 class="text-2xl font-bold text-green-900">Update Recipient</h2>
                    <p class="text-green-600 text-sm mt-1 font-medium">Currently Editing: <span class="text-green-900"><?= htmlspecialchars($recipient['first_name'] . ' ' . $recipient['last_name']); ?></span></p>
                </div>
                <div class="shrink-0 flex flex-col items-end gap-2">
                    <?php if (!empty($recipient['profile_image'])): ?>
                        <img src="../../../uploads/<?= htmlspecialchars($recipient['profile_image']); ?>" class="w-16 h-16 rounded-full object-cover border-2 border-white shadow-md ring-1 ring-green-100">
                    <?php else: ?>
                        <div class="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center text-green-600 font-bold border-2 border-white shadow-sm">
                            <?= substr($recipient['first_name'], 0, 1) . substr($recipient['last_name'], 0, 1); ?>
                        </div>
                    <?php endif; ?>
                    <span class="text-[10px] font-black uppercase tracking-widest text-green-800 opacity-40">Recipient ID #<?= $recipient['recipient_id']; ?></span>
                </div>
            </div>
                ...
                <form action="AdminRecipientStore.php" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div>
                        <h3 class="text-sm font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                            <span class="w-2 h-2 bg-slate-300 rounded-full"></span> Account Details (Read-only)
                        </h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-gray-50 border border-gray-100 p-4 rounded-xl">
                                ...
                                <input type="text" value="<?= htmlspecialchars($recipient['username']); ?>" disabled
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-100 text-gray-500 italic cursor-not-allowed">
                            </div>
                            <div class="bg-gray-50 border border-gray-100 p-4 rounded-xl">
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1">Email Address</label>
                                <input type="email" value="<?= htmlspecialchars($recipient['email']); ?>" disabled
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-100 text-gray-500 italic cursor-not-allowed">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pt-6 border-t border-green-50">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="first_name" placeholder="<?= htmlspecialchars($recipient['first_name']); ?>"
                            ...
                            <div>
                                ...
                                <input type="text" name="last_name" placeholder="<?= htmlspecialchars($recipient['last_name']); ?>"
                                    class="w-full px-4 py-3 border border-green-100 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition bg-white">
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-green-800 mb-2">New Preferences</label>
                                <textarea name="preferences" rows="2" placeholder="<?= !empty($recipient['preferences']) ? htmlspecialchars($recipient['preferences']) : 'Enter new preferences...'; ?>"
